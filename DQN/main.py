...
for episode in range(num_episodes):  # 試行数分繰り返す
    ...
    state, reward, done, done2, _ = env.step(env.action_space.sample())  # 1step目は適当な行動をとる
    ...
    for t in range(max_number_of_steps + 1):  # 1試行のループ
        ...
        next_state, reward, done, done2, info = env.step(action)   # 行動a_tの実行による、s_{t+1}, _R{t}を計算する
